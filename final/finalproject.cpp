...
// Processing
//double calculateMortgage(double apr, int term, double loan);
double sumElements(double arrayNeed[NEEDS], vector<double> arrayWant);
//void selectionSort(double arrayNeed[NEEDS], string need[NEEDS], double needPercent[NEEDS]);
void calculatePercentage(double arrayNeed[NEEDS], vector<double> arrayWant, double needPercent[NEEDS], vector<double> wantPercent, double sum);
// Output
//void printResults(ostream &stream, string need[NEEDS], vector<string> want, double needPercent[NEEDS], vector<double> wantPercent);
...
//
// Inputs data into apr term loan variables as well as the vector and array
//
void getData(double &apr, int &term, double &loan, double arrayNeed[NEEDS], string need[NEEDS], vector<double> &arrayWant, vector<string> &want)
{
...
double sumElements(double arrayNeed[NEEDS], vector<double> arrayWant)
{
    ...
    for (vector<double>::iterator j = arrayWant.begin(); j != arrayWant.end(); j++)
    {
        sum += *j;
    }
...
void calculatePercentage(double arrayNeed[NEEDS], vector<double> arrayWant, double needPercent[NEEDS], vector<double> wantPercent, double sum)
{
    ...
    {
        cout << needPercent[j] << endl;
    }

    for (int k = 0; k < arrayWant.size(); k++)
    {
        wantPercent.push_back((arrayWant[k] / sum) * 100);
    }

    cout << "Want %" << endl;
    for (vector<double>::iterator l = wantPercent.begin(); l != wantPercent.end(); l++)
    {
        cout << *l << endl;
    }
}
